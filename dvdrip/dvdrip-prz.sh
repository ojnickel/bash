...
size_video (){ #länge(sekunde),bitrate
	sizev=$(( $1 * $2 ))
}
...
if [ ! $# == 3 ]; then
	...
	duration=`mkvinfo $2 | grep -i dauer | cut -d " " -f 4 | grep s | cut -d "s" -f 1`
	richtig="n"	
	zeit_faktor=1
	echo "Getting crop"
	while [ "$richtig" == "n" ]
...
#		`mplayer $2  -aid $AUDIO -vo null -vc null -ao pcm:fast:file=$AUDIO.wav 2>&1 > /dev/null`
		...
		#prozent bar
		#wav calculation
		srate=`mediainfo ${audio[$i]}.wav | grep -i sampl | cut -d ":" -f 2 | cut -d  " " -f 2 `
		channels=`mediainfo ${audio[$i]}.wav | grep -i channe | cut -d ":" -f 2 | cut -d  " " -f 2 `
		bits=`mediainfo ${audio[$i]}.wav | grep -i "bit d" | cut -d ":" -f 2 | cut -d  " " -f 2 `
		
		#estimated size
		size_wav $srate $channels $bits $duration #-> große in bytes
		current=1
		while [  $current -lt $sizea ]; do
			`~/prog/bash/prozent/prozent.sh $sizea $current`
			sleep 1
			size_c "${audio[$i]}.wav"
		done
