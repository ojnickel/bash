...
#audio von mkv
audio_m () { #film, anzahlt tracks
	if [ ! $2 == 0 ]; then
		anzahl_a=$(mplayer2 $1 -vo null -frames 1 | grep mkv | grep audio | wc -l  )
		atracks=$(mplayer2 $1 -vo null -frames 1 | grep mkv | grep audio
...
cur_laenge () { #output
	...
	in=`realpath $1.264`
	in="$pwd$1.264"
	akt_laenge=`ls -s $1.264 | cut -d" " -f 1`
}
...
if [ "$6" == "2" ]; then
	...
	#echo $mkv_size
	sleep 2 #wait for mencoder to create .264
...
#	echo $run
	...
	do
		...
		/home/oswaldo/prog/bash/bdrip/prozent-1.1.sh $mkv_size $akt_laenge $elapsed $bit
		process "mencoder"
		old_laenge=$akt_laenge
		sleep  1
		cur_laenge  "$2"
...
#get # audio streams
	...
	do
		...
		read lang
		srt[$i]=$lang
		echo "Enter filename for ${sublang[$i]}:"
		read slang
		ext[$i]=$slang
		#s.o. 
		#fängt ab audiospuren an
